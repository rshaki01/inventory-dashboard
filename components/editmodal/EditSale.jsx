...
const EditSale = ({sale, open, onClose}) => {

  if (!open) return null;
  const {editSale} = useContext(GlobalContext);
  const [name, setName] = useState(sale.name);
  const [purchasePrice, setPurchasePrice] = useState(sale.purchasePrice);
  const [listingPrice, setListingPrice] = useState(sale.listingPrice)
  const [quantity, setQuantity] = useState(sale.quantity);

  const handleSubmit = (e) => {
    e.preventDefault();
    let id = sale.id
    let newSale = {id, name, purchasePrice, listingPrice, quantity};
    editSale(newSale);
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        ...
        <div className="flex justify-between">
            <h1 className="text-slate-50 text-xl pb-4">Edit Sale</h1>
